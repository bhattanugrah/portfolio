...
export const Menu = ({
    ...
    return (
        <HoverBorderGradient
            className="relative rounded-full bg-black shadow-input flex justify-center space-x-4 px-8 py-6 "
            as={"button"}
        >
            {children}
        </HoverBorderGradient>
...
export const HoveredLink = ({ children, ...rest }: any) => {
    return (
        <Link
            {...rest}
            className="text-neutral-700 text-neutral-200 hover:text-white "
        >
            {children}
        </Link>
    );
